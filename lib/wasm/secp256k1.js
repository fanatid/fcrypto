module.exports = ({ fns, heapu8, heap32 }) => {
  return class Secp256k1 {
    ...
    privateKeyTweakAdd (seckey, tweak) {
      // While we use ptr64 for tweak, there actually only 32 bytes used
      try {
        heapu8.set(seckey, this.ptr32)
    ...
    ecdsaSign (obj, msg32, seckey) {
      // While we use ptr64 for msg32, there actually only 32 bytes used
      try {
    ...
    ecdh (output, pubkey, seckey) {
      // While we use ptr64 for hash output, there actually only 32 bytes used
      try {
        heapu8.set(pubkey, this.ptr72)
        heapu8.set(seckey, this.ptr32)
